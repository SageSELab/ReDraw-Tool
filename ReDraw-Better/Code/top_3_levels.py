        ...
        if p not in new_hierarchy_nodes:
            ...
            if equal==True:
                ...
                if "bounds" in parent.attrib:
                    ...
                    parent.clear()
                    parent.append(common_nodes[i])
                    bounds=common_nodes[i].attrib["bounds"]
                    parent.attrib["bounds"] = bounds
                    parent.attrib["xStart"]=bounds[0]
                    parent.attrib["yStart"]=bounds[1]
                    parent.attrib["xEnd"]=bounds[2]
...
def get_top_3_levels(list_):
    temp=[len(list_[i]) for i in range(len(list_))]
    idx = sorted(range(len(temp)),key=lambda i: temp[i], reverse=True)[:3]
    return idx
...
def main(target_xml,xml_folder):
    ...
    for i in range(3):
        ...
        for files in fileList:
            ...
            top3_levels = get_top_3_levels(levels)
            dataset_leafs=levels[top3_levels[i]]
